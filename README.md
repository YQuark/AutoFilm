...
## 核心能力
当前版本聚焦 Alist2Strm：扫描 Alist 目录生成 `.strm` 文件，并可按配置下载图片、字幕、NFO 与自定义后缀文件。

## Alist2Strm 增量扫描
`incremental: True` 默认启用文件级增量，会持久化扫描清单并跳过未变更文件的生成/下载。默认级别为 `file`，仍会遍历远端目录树，以保证删除同步和后端目录时间不可靠时不漏扫。

如果 Alist 后端的目录修改时间会随子文件变化而更新，可以显式开启目录级增量：
```yaml
incremental: True
incremental_level: directory
```

目录级增量会在目录未变更时跳过整个子树，减少 `/api/fs/list` 调用。若本轮跳过了目录且 `sync_server: True`，程序会跳过本地删除清理，避免把未遍历子树误判为远端删除。目录修改时间不可靠的存储建议继续使用默认 `file`，或拆小 `source_dir`。
...
当前版本不再内置 Web Token 鉴权；任务触发和配置写接口不需要 `Authorization` 请求头。若 Web UI / API 暴露到可信网络以外，建议放在反向代理、VPN 或其他访问控制之后。

v2.1.0 起 Web UI 升级为运维控制台，支持任务仪表盘、运行历史、配置摘要、主题/密度/视图偏好，以及带备份和校验的 `config.yaml` 编辑。当前版本配置写操作会直接校验、备份并保存 `config.yaml`，访问控制请在外层网络边界实现。
...
- [x] 任务执行超时与优雅关闭
- [ ] `.strm` 文件 URL 可用性定期校验
- [ ] 图片、NFO、字幕下载失败统计与重试可视化
- [ ] 通知 Webhook 自定义请求方法（支持 PUT 等）
- [ ] 移动端 Web 控制台适配

# 更新日志
- 2026.5.17：v2.3.0，全面代码审计与优化升级：关键修复（AlistClient Token-Only 认证 AttributeError、Release 工作流 CI Gate、优雅关闭 SIGTERM/SIGINT 处理、任务状态永久卡死、通知失败中断任务执行、X-Forwarded-For 取错客户端 IP、BDMV 路径大小写敏感）；代码质量（`run()` 方法拆分为 6 个职责清晰的方法、移除 LoggerManager 包装器 -80 行、清理 Ani2AlistList 等死代码与笔误、重构 Singleton/Multiton 为线程安全、重试退避改为指数）；运维加固（Docker healthcheck、日志 backupCount 从 0 改为 30 天轮换、异步端点同步 I/O 修复、CI 新增 mypy + bandit、Alist 凭证支持环境变量覆盖 `AUTOFILM_ALIST_{ID}_TOKEN`、Docker 镜像标签固定版本号）；Web 增强（通知失败隔离保护、配置模式未识别时显式警告）。
- 2026.5.14：v2.2.1，移除 Ani2Alist 模块及其 RSS / UrlTree 追番链路，项目能力收敛为 Alist2Strm 扫描生成 `.strm` 并下载图片、字幕、NFO、自定义后缀文件；配置示例、Web 摘要、依赖与 Roadmap 同步清理。
- 2026.5.13：v2.2.0，全面代码审计升级：核心修复（配置 mtime 缓存、name mangling、过滤器子串误伤、iter_path 崩溃恢复、清理废弃 LibraryPosterList 配置）；Web 安全加固（XSS onclick 注入修复、移除内置 Web Token 鉴权、请求限流、请求体上限）；Web 美观升级（侧边栏 SVG 图标、状态动画、表格悬停交替色、日志级别着色、Metric 彩色顶条、空状态容器、最近活动可点击）；Web 功能增强（任务搜索/列排序/详情弹窗、备份删除与轮换、批量运行/重试失败、YAML 编辑器语法高亮/脏检查、diff 保存确认、Toast 通知、按钮 loading 状态、快捷键、日志查看器）；配置增强（ruamel.yaml 保留注释往返、动态模块筛选）；README TODO → Roadmap
- 2026.5.11：v2.1.2，Alist2Strm 新增可选目录级增量扫描 `incremental_level: directory`，未变更目录不再递归进入；扫描日志拆分目录扫描数、跳过目录数、发现文件数和待处理文件数；目录级跳过命中时跳过本轮本地删除清理，避免 `sync_server` 误删未遍历子树；README 与配置示例补充目录级增量风险说明。
- 2026.5.11：v2.1.1，Web 控制台页面可见文案统一中文化，包含导航、按钮、状态、提示、表格列名、运行历史、配置编辑与错误提示。
