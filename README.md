...
# 更新日志
- 2026.5.13：v2.2.0，全面代码审计升级：核心修复（配置 mtime 缓存、name mangling、过滤器子串误伤、iter_path 崩溃恢复、清理废弃 LibraryPosterList 配置）；Web 安全加固（XSS onclick 注入修复、Token 会话级存储/时序安全比较、X-Forwarded-For 防伪造限流、请求体上限）；Web 美观升级（侧边栏 SVG 图标、状态动画、表格悬停交替色、日志级别着色、Metric 彩色顶条、空状态容器、最近活动可点击）；Web 功能增强（任务搜索/列排序/详情弹窗、备份删除与轮换、批量运行/重试失败、YAML 编辑器语法高亮/脏检查、diff 保存确认、Toast 通知、按钮 loading 状态、快捷键、日志查看器）；配置增强（ruamel.yaml 保留注释往返、示例 Token 占位符、动态模块筛选、Token 管理迁移设置页）；README TODO → Roadmap
